...
class child1 extends Component {
    ...
    componentDidUpdate() {
        ...
        const x_scale = d3.selectLinear().domain([0, d3.max(x_data)]).range([margin.left, w]);
        ...
        const y_scale = d3.selectLinear().domain([0, d3.max(y_data)]).range([h, 0]);
        container.selectAll(".y_axis_g").data([0]).join('g').attr("class", "x_axis_g")
            .attr("transform", 'tanslate(${margin.left},0)').call(d3.axisLeft(y_scale));
